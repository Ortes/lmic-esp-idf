...
static void updateTx (ostime_t txbeg) {
    ...
    if( LMIC.globalDutyRate != 0 )
        ...
        lmic_printf("%ld: Updating info for TX at %ld, airtime will be %ld. Setting available time for band %lu to %lu\n", os_getTime(), txbeg, airtime, freq & 0x3, band->avail);
        if( LMIC.globalDutyRate != 0 )
            lmic_printf("%ld: Updating global duty avail to %ld\n", os_getTime(), LMIC.globalDutyAvail);
    #endif
}
    ...
    do {
        ...
        for( u1_t bi=0; bi<4; bi++ ) {
            if( (bmap & (1<<bi)) && mintime - LMIC.bands[bi].avail > 0 ) {
                #if LMIC_DEBUG_LEVEL > 1
                    lmic_printf("%ld: Considering band %d, which is available at %lu\n", os_getTime(), bi, LMIC.bands[bi].avail);
                #endif
        ...
        #if LMIC_DEBUG_LEVEL > 1
            lmic_printf("%ld: No channel found in band %d\n", os_getTime(), band);
        #endif
        if( (bmap &= ~(1<<band)) == 0 ) {
...
static ostime_t nextJoinState (void) {
    ...
    #if LMIC_DEBUG_LEVEL > 1
        if (failed)
            lmic_printf("%ld: Join failed\n", os_getTime());
        else
            lmic_printf("%ld: Scheduling next join at %ld\n", os_getTime(), LMIC.txend);
    #endif
...
static bit_t decodeFrame (void) {
    ...
    if( dlen < OFF_DAT_OPTS+4 ||
      ...
      norx:
        ...
        lmic_printf("%ld: Invalid downlink, window=%s\n", os_getTime(), window);
#endif
        LMIC.dataLen = 0;
        return 0;
    }
...
#if LMIC_DEBUG_LEVEL > 0
    lmic_printf("%ld: Received downlink, window=%s, port=%d, ack=%d\n", os_getTime(), window, port, ackup);
#endif
    return 1;
}
...
static void engineUpdate (void) {
#if LMIC_DEBUG_LEVEL > 0
    lmic_printf("%ld: engineUpdate, opmode=0x%x\n", os_getTime(), LMIC.opmode);
#endif
    ...
    if( (LMIC.opmode & (OP_JOINING|OP_REJOIN|OP_TXDATA|OP_POLL)) != 0 ) {
        ...
        #if LMIC_DEBUG_LEVEL > 1
            if (jacc)
                lmic_printf("%ld: Uplink join pending\n", os_getTime());
            else
                lmic_printf("%ld: Uplink data pending\n", os_getTime());
        #endif
        ...
        if( (LMIC.opmode & OP_NEXTCHNL) != 0 ) {
            ...
            LMIC.opmode &= ~OP_NEXTCHNL;
            #if LMIC_DEBUG_LEVEL > 1
                lmic_printf("%ld: Airtime available at %ld (channel duty limit)\n", os_getTime(), txbeg);
            #endif
        } else {
            txbeg = LMIC.txend;
            #if LMIC_DEBUG_LEVEL > 1
                lmic_printf("%ld: Airtime available at %ld (previously determined)\n", os_getTime(), txbeg);
            #endif
        }
        ...
        if( (LMIC.globalDutyRate != 0 || (LMIC.opmode & OP_RNDTX) != 0)  &&  (txbeg - LMIC.globalDutyAvail) < 0 ) {
            ...
            #if LMIC_DEBUG_LEVEL > 1
                lmic_printf("%ld: Airtime available at %ld (global duty limit)\n", os_getTime(), txbeg);
            #endif
        }
#if !defined(DISABLE_BEACONS)
        ...
        if( (LMIC.opmode & OP_TRACK) != 0 &&
            ...
            #if LMIC_DEBUG_LEVEL > 1
                lmic_printf("%ld: Awaiting beacon before uplink\n", os_getTime());
            #endif
        ...
        if( txbeg - (now + TX_RAMPUP) < 0 ) {
            #if LMIC_DEBUG_LEVEL > 1
                lmic_printf("%ld: Ready for uplink\n", os_getTime());
            #endif
        ...
        #if LMIC_DEBUG_LEVEL > 1
            lmic_printf("%ld: Uplink delayed until %ld\n", os_getTime(), txbeg);
        #endif
        // Cannot yet TX
        if( (LMIC.opmode & OP_TRACK) == 0 )
